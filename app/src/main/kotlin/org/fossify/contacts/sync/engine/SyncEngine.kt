package org.fossify.contacts.sync.engine
...
import org.fossify.contacts.sync.db.BlindIndexEntity
import org.fossify.contacts.sync.db.BlobStateEntity
import org.fossify.contacts.sync.db.SyncDatabase
import org.fossify.contacts.sync.db.SyncRecordEntity
import org.fossify.contacts.sync.db.SyncStateEntity
import org.fossify.contacts.sync.model.ContactPayload
import org.fossify.contacts.sync.model.ContactPayload.Companion.toLocalContact
import org.fossify.contacts.sync.model.Merger
import org.fossify.contacts.sync.net.SyncApi
...
class SyncEngine(private val context: Context) {
    ...
    private val dao = SyncDatabase.get(context).syncDao()
    ...
    private fun fail(message: String): Report {
        dao.putState((dao.getState() ?: SyncStateEntity()).copy(lastError = message))
    ...
    private fun detectLocalChanges(dek: ByteArray, api: SyncApi) {
        ...
        // sync_records 里有但本机已经没有的 → 用户删掉了，推墓碑上去
        for (known in knownByLocalId.values) {
            if (known.localId !in seenLocalIds && !known.deletedLocally) {
                dao.upsert(
                    known.copy(
                        deletedLocally = true, dirty = true, localId = 0,
